...
export const Button = ({variant='primary',size='medium',loading=false,leftIcon,rightIcon,children,className,...props}:ButtonProps) => {
 ...
 return <button {...props} disabled={props.disabled||loading} aria-busy={loading||undefined} className={classNames('relative inline-flex max-w-full items-center justify-center gap-2 rounded-xl border text-center font-medium transition focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50',size!=='icon'&&'whitespace-normal break-words',variants[variant],sizes[size],className)}>{loading&&<Loader2 aria-hidden className="h-4 w-4 animate-spin"/>}{!loading&&leftIcon}<span className={classNames('min-w-0',size==='icon'&&'sr-only')}>{children}</span>{!loading&&rightIcon}</button>;
};
